...
互斥,且按骨骼整体清空),其中"总是同时在播"的冲突在移植期静态删通道(`DedupPreLayer` /
常量变体折叠),其余(含主链压 pre 层)由 `ApplyChannelOwnership` 按状态动态让出(§9)。早先按"常驻 pre 与 idle
总在同播"静态删通道的 `ApplyJavaMainOverride` 已撤: 走路/卡片预览不播 idle 时被删的隐藏缩放会漏出来(大酒狐爱心/ZZZ)。
...
- 替换实体(`files.projectiles`/`vehicles`)的通道语义与写入语义:载具按 Java 第一乘客写入、下车不撤、随实体存档
  ...
  `ysm_vehicle_root` 根骨骼;**船/运输船是引擎硬编码渲染**(改单实体渲染就整条不显示),改用客户端替身实体
  (`client/render/vehicleRender.py`,基岩船偏航比 Java 口径多 90°);载具动画 `sound_effects` 照常登记
...
- 替换实体:乘客座位定位骨骼(`PassengerLocator*`,Java 3.0-dev 里本身还是 TODO)、载具 `roaming` 变量不随主人复制、
  船的替身按逻辑 tick 插值跟位(高速行驶的跟手程度以实机为准)、矿车不复刻轨道坡度与受击摇晃(Java 同样没有)
