...
> pre 层内部同理由 `DedupPreLayer` 按 Java 通道序去重（`pre_parallel_0..7 → vehicle →
> pre_main`，同状态内后者胜出）：能同时在播的两条共写一个（骨骼, 通道）时只留序靠后的那条，
> 常驻与门控都参与（凋灵娘第 2 通道的眼型压掉第 1 通道整套步态的眼光定位 37 对，与 Java
> 一致）；同一控制器的不同状态、同一状态内互斥的 dict 条目不互剥；被控制器接管的裸
> `pre_parallelN` 只按控制器里那条的序算，不再另记"直播"幻影条目。
